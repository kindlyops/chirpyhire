...
RSpec.describe Organizations::SubscriptionsController, type: :controller do
  ...
  describe '#create' do
    ...
    context 'with a person' do
      ...
      context 'without a lead' do
        it 'creates a subscribed lead' do
          expect {
            post :create, params: params
          }.to change { person.leads.count }.by(1)
        end
      end
  ...
  describe '#destroy' do
    ...
    context 'with a person' do
      ...
      context 'with a subscribed lead' do
        let!(:lead) { create(:lead, person: person, organization: organization) }
        it 'unsubscribes the lead' do
          expect {
            delete :destroy, params: params
          }.to change { lead.reload.subscribed? }.from(true).to(false)
        end
      end
    end
